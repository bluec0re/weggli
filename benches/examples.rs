...
fn run_query(s: &(tree_sitter::Tree, String), pattern: &str) {
    let tree = weggli::parse(pattern, weggli::LanguageMode::C);

    let mut c = tree.walk();
    c.goto_first_child();
    let qt =
        weggli::builder::build_query_tree(pattern, &mut c, weggli::LanguageMode::C, None).unwrap();

    let matches = qt.matches(s.0.root_node(), &s.1);
    for m in matches {
        m.display(&s.1, 500, 500, false, None);
    }
}
...
fn bench(c: &mut Criterion) {
    let p = |path| {
        ...
        (weggli::parse(&source, weggli::LanguageMode::C), source)
    };
